    ...
    <>
      ...
      >
          ...
          <section className="workspace-panel overflow-hidden">
            ...
            <div><table className="w-full text-left text-sm"><thead><tr className="border-b bg-slate-50 text-xs uppercase text-slate-500">{["№ заказа", "Клиент", "Тип изделия", "Материал", "Текущий этап", "Мастер", "Дата передачи", "Плановая готовность", "Осталось дней", "Комментарий", "Действия"].map((item, index) => <th key={item} className={`px-4 py-3 ${[2, 3, 6, 9].includes(index) ? "hidden 2xl:table-cell" : ""}`}>{item}</th>)}</tr></thead><tbody>{filtered.map((order) => { const days = daysUntil(order.deadline); return <tr key={order.id} className="border-b last:border-0"><td className="px-4 py-4"><Link href={`/orders/${order.id}`} className="font-semibold text-brand-700 hover:underline">{order.id}</Link></td><td className="px-4 py-4 font-medium">{order.client}</td><td className="hidden px-4 py-4 text-slate-600 2xl:table-cell">{order.product}</td><td className="hidden px-4 py-4 text-slate-600 2xl:table-cell">{order.material}</td><td className="px-4 py-4"><select className="input h-9 min-w-40" value={order.stage} onChange={(event) => changeStage(order.id, event.target.value as ProductionStage)}>{productionStages.map((item) => <option key={item}>{item}</option>)}</select></td><td className="px-4 py-4"><select className="input h-9 min-w-44" value={order.masterId} onChange={(event) => updateProduction(order.id, { masterId: event.target.value }, "Мастер назначен")}>{masters.map((item) => <option key={item.id} value={item.id}>{item.fullName}</option>)}</select></td><td className="hidden px-4 py-4 2xl:table-cell">{order.transferDate}</td><td className="px-4 py-4"><input className="input h-9 min-w-36" type="date" value={order.deadline} onChange={(event) => updateProduction(order.id, { plannedReadyAt: event.target.value }, "Плановая готовность изменена")} /></td><td className={`px-4 py-4 font-bold ${days < 0 ? "text-red-600" : days <= 3 ? "text-orange-600" : "text-slate-700"}`}>{days < 0 ? `${Math.abs(days)} дн. просрочено` : `${days} дн.`}</td><td className="hidden px-4 py-4 2xl:table-cell"><input className="input h-9 min-w-64" value={order.comment} onChange={(event) => updateProduction(order.id, { comment: event.target.value }, "Комментарий производства обновлен")} /></td><td className="px-4 py-4"><Link href={`/orders/${order.id}`} className="btn-secondary h-9 whitespace-nowrap">Открыть заказ</Link></td></tr>; })}</tbody></table></div>
          </section>
      </AppShell>
