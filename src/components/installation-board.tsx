...
import {
  AlertTriangle, CalendarDays, CheckCircle2, ChevronDown, Eye,
  ...
  return (
    <>
      ...
      >
          ...
          <div className="mb-6 flex rounded-xl border bg-white p-1 shadow-card"><button className={`flex-1 rounded-lg px-4 py-3 text-sm font-semibold ${view === "list" ? "bg-brand-600 text-white" : "text-slate-500 hover:bg-slate-50"}`} onClick={() => setView("list")}>Список</button><button className={`flex-1 rounded-lg px-4 py-3 text-sm font-semibold ${view === "calendar" ? "bg-brand-600 text-white" : "text-slate-500 hover:bg-slate-50"}`} onClick={() => setView("calendar")}>Календарь</button></div>
